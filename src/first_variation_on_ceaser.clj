...
(defn movingShift [s shift]
  (let [encoded (map-indexed
                  (fn [i c]
                    (let [code (int c)
                          shift-value (+ code i shift)]
                      (cond
                        (and (>= code 97) (<= code 122))
                        (char (mod (- shift-value 97) 26 + 97))

                        (and (>= code 65) (<= code 90))
                        (char (mod (- shift-value 65) 26 + 65))

                        :else
                        c)))
                  s)
        average-len (int (/ (count encoded) 5))
        part-lengths (->> (repeatedly #(if (< % 4) (inc average-len) average-len) 5)
                          (concat (repeat (- 5 (count encoded))))
                          (map int))]
    (partition part-lengths encoded)))

(defn demovingShift [s shift]
  (apply str
         (map
           (fn [c i]
             (let [code (int c)
